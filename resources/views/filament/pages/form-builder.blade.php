<x-filament-panels::page>
                                                ...
                                                <div class="fb-field-actions">
                                                    @if (in_array($field->type, ['select', 'radio', 'checkbox']))
                                                        <x-filament::icon-button
                                                            ...
                                                            tag="a"
                                                            :href="route('filament.admin.resources.form-field-options.index', ['tableFilters[form_field_id][value]' => $field->id])"
                                                        />
                                                    @endif
    ...
    @push('scripts')
        ...
        <script>
                ...
                document.querySelectorAll('.field-dropzone').forEach((zone) => {
                    ...
                    new Sortable(zone, {
                        ...
                        onAdd(evt) {
                            ...
                            @this.addField(parseInt(sectionId), type, label);
                            evt.item.remove();
                        },
                        onEnd() {
                            const fields = [];
            ...
            if (window.Livewire) {
                Livewire.on('loadTemplate', () => {
                    setTimeout(initFormBuilder, 50);
                });

                Livewire.on('openEditFieldModal', () => {
                    window.dispatchEvent(new CustomEvent('open-modal', {
                        detail: { id: 'edit-field-modal' },
                    }));
                });

                Livewire.on('closeEditFieldModal', () => {
